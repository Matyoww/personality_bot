...
if "chosen_personality" not in st.session_state:
    st.session_state.chosen_personality = pick_personality()

st.set_page_config(page_title="Chatbot", page_icon="🤖", layout="wide")
st.title("Chat with a bot with different personalities! 🤖")

with st.sidebar:
    if st.session_state.chosen_personality:
        st.title(f"Personality: {st.session_state.chosen_personality}")
